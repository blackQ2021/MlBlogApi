...
public class EmailSender {
    ...
    private EmailSender() {
        EmailSender.config(EmailSender.SMTP_QQ(false), "2595762362@qq.com", "qmtplsrqobxoebag");
    }
    ...
    public static void sendCommentNotify(String notifyContent, String address) throws Exception {
        EmailSender.subject("ml博客系统-评论通知")
                .from("ml博客系统")
                .text(notifyContent)
                .to(address)
                .send();
...
//     发送邮件
    public static void sendRegisterVerifyCode(String code, String address) throws Exception {
        EmailSender.subject("ml博客系统注册验证码")
                .from("ml博客系统")
                .text("你的验证码是" + code + "请在10分钟内完成注册，非本人操作请忽略。")
                .to(address)
                .send();
    }
}
